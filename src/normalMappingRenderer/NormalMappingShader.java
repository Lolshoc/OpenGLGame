...
public class NormalMappingShader extends ShaderProgram{
	...
	protected void loadClipPlane(Vector4f plane){
		super.loadVecter(location_plane, plane);
	}
	...
	protected void loadLights(List<Light> lights, Matrix4f viewMatrix){
		for(int i=0;i<MAX_LIGHTS;i++){
			if(i<lights.size()){
				super.loadVecter(location_lightPositionEyeSpace[i], getEyeSpacePosition(lights.get(i), viewMatrix));
				super.loadVecter(location_lightColour[i], lights.get(i).getColour());
				super.loadVecter(location_attenuation[i], lights.get(i).getAttenuation());
			}else{
				super.loadVecter(location_lightPositionEyeSpace[i], new Vector3f(0, 0, 0));
